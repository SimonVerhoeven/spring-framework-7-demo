...
The `spring-context` module has a `@ConcurrencyLimit` annotation which is aligned with `core.retry`, alongside a `ConcurrencyLimit` which is based on the concurrency throttling support.
...
For more details I recommend checkout out the https://docs.spring.io/spring/reference/7.0/core/resilience.html[Resilience reference documentation].
