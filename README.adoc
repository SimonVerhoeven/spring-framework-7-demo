...
=== New JmsClient and revisited JdbcClient

The new `JmsClient` handles common send and receive operations against a JMS destination, working seamlessly with Spring's common Message abstraction and aligning error handling (throwing `MessagingException`) with the spring-messaging module.

Essentially, it serves as an alternative to the older JmsMessagingTemplate, delegating actual execution to the core `JmsTemplate`.

The new `JmsClient` allows us to configure custom QoS settings through reusable operations. To stay in sync, the existing `JdbcClient` now conveniently provides statement-level settings such as fetch size, max rows, and query timeout.
...
=== New resilience features: RetryTemplate, @Retryable, @ConcurrencyLimit

The Spring Team has been trimming the fat from the `Spring Retry` project, and revisited some APIs.
Subsequently, they've merged the result into spring-core module, where we can now find the foundational support in the `org.springframework.core.retry` package.
Here we can find classes such as `RetryTemplate`, `RetryPolicy` and the supporting classes.

The `spring-context` module has a `@Retryable` annotation which is aligned with `core.retry`, alongside a `@ConcurrencyLimit` annotation which is based on Spring's concurrency throttling support.
Both of these can be enabled through the `@EnableResilientMethods` annotation on a @Configuration class.

Those using a reactive stack will be happy to learn that `@Retryable` and its customization through annotation attributes automatically adapts to reactive methods with a reactive return type by decorating the pipeline with Reactor's retry capabilities. Imperative methods will be called through a `RetryTemplate` with a corresponding `RetryPolicy`.

For more details I recommend checking out the https://docs.spring.io/spring/reference/7.0/core/resilience.html[Resilience reference documentation].
...
=== JPA 3.2 adaptation (and Hibernate ORM 7.2)

As of this release `LocalEntityManagerFactoryBean` accepts a JPA 3.2 `PersistenceConfiguration` which enables richer bootstrapping without having to fall back to a `persistence.xml` file.
If we pass in a `PersistenceConfiguration` to a `LocalContainerEntityManagerFactoryBean` it will be merged into its default persistence unit.
In case of a `HibernatePersistenceConfiguration` the Hibernate 7.1 scanning configuration options will be detected as well.

Transactional `Session` and `StatelessSession` proxy references are also exposed by `LocalSessionFactoryBean` for dependency injection, similar to the JPA 3.2 setup for `EntityManager` injection with `@Inject` or `@Autowired`.
In case you decide to use `StatelessSession` it's recommended to use Hibernate ORM 7.2 and not 7.1, since this will allow a transactional `StatelessSession` to be derived from the current transactional `Session`.
...
It can be bound to a live server, an MVC, or the application context.
